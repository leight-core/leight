...
export function createPostHook<TQuery extends IQueryParams = IQueryParams, TRequest = any, TResponse = any>(link: string): IRequestHookCallback<TQuery, TRequest, TResponse> {
	return (request: TRequest, query?: TQuery, config?: AxiosRequestConfig): IRequestHookResult<TResponse> => {
		...
		const result = useQuery([link, {query, config}], () => {
			return new Promise<TResponse>((resolve, reject) => {
				axiosHandler(config => axios.post<TResponse>(discoveryContext ? discoveryContext.link(link, query) : linkContext.generate(link, query), request, config), config)
					.on("response", resolve)
					.on("error", reject)
					.chain(events);
			});
